    ...
    try {
        ...
        for (const email of emails) {
            try {
                ...
                const emailWithAttachments = await googleService.getEmailWithAttachments(
                    email.id,
                    attachmentService,
                    lockedAttachmentsPassword ? lockedAttachmentsPassword : undefined
                );
                ...
                results.push({
                    ...
                    attachmentAnalyses: result.attachmentAnalyses?.map((analysis: any) => ({
...
router.get(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const data =
                await financialDataService.getFinancialData(
                    userId,
                    {
                        startDate: startDate
        ...
        try {
            ...
            if (!userId || !q) {
                res.status(400).json({
                    success: false,
                    message:
                        'User ID and search query are required',
                });
                return;
            }

            const results =
                await financialDataService.searchFinancialData(
                    userId,
                    String(q)
                );
...
router.get(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const stats =
                await financialDataService.getStatistics(
                    userId,
                    parseInt(String(monthsBack || 6))
                );
...
router.get(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const transaction =
                await financialDataService.getTransactionDetails(
                    transactionId,
                    userId
                );
...
router.delete(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const deleted =
                await financialDataService.deleteTransaction(
                    transactionId,
                    userId
                );
