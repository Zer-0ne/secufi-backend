...
export class GmailAttachmentService {
   ...
   */
  async extractAttachmentMetadata(
    emailMessage: gmail_v1.Schema$Message
  ): Promise<Attachment[]> {
    try {
   ...
   */
  private extractPDFFromBuffer(buffer: Buffer): string {
    try {
   ...
   */
  private extractCSVFromBuffer(buffer: Buffer): string {
    try {
   ...
   */
  private extractExcelFromBuffer(buffer: Buffer): string {
    try {
      const text = buffer
        .toString('utf-8')
        .split('\0')
   ...
   */
  private extractDOCXFromBuffer(buffer: Buffer): string {
    try {
   ...
   */
  private extractImageFromBuffer(buffer: Buffer, filename: string): string {
...
Processing status: Ready for OCR analysis`;
  ...
  isFinancialDocument(filename: string, mimeType: string): boolean {
    ...
    return (
      financialKeywords.some((keyword) => lower.includes(keyword)) ||
   ...
   */
  private async checkPdfProtection(tmpFile: string): Promise<boolean> {
  ...
  private async callPythonExtractor(
    ...
    filename: string,
    password?: string
  ): Promise<{ success: boolean; text: string; metadata: Record<string, any> }> {
   ...
   */
  private async extractContentFromBuffer(
    buffer: Buffer,
    mimeType: string,
    filename: string,
    password?: string
    ...
    try {
      ...
      if (mimeType === 'application/pdf' || ext === '.pdf') {
        ...
        console.log(
          `⚠ Python extraction failed or returned low quality, using fallback for ${filename}`
        );
   ...
   */
  async downloadAndProcessAttachment(
    gmail: gmail_v1.Gmail,
    emailId: string,
    ...
    try {
      // Download from Gmail
      const buffer = await this.downloadAttachmentStream(
        gmail,
        emailId,
        attachment.id,
        attachment.filename
      ...
      const content = await this.extractContentFromBuffer(
        buffer,
        attachment.mimeType,
        attachment.filename,
        password!
   ...
   */
  async downloadAndProcessAttachments(
    gmail: gmail_v1.Gmail,
    emailId: string,
  ...
  > {
    ...
    for (const attachment of attachments) {
      try {
        const processed = await this.downloadAndProcessAttachment(
          gmail,
          emailId,
          attachment,
          userId,
