 ...
 */
const getLocationFromIPGuide = (ip) => {
    return new Promise((resolve, reject) => {
...
deviceInfoRouter.get('/', async (req, res) => {
    ...
    const isPrivate = ipv4 ? (ipv4.startsWith('192.168.') ||
        ipv4.startsWith('10.') ||
        ipv4.startsWith('172.16.') ||
        ipv4.startsWith('172.31.')) : false;
    // Get location data
    let location = {};
    try {
        location = await getLocationFromAPI(ipv4 || clientIp);
    }
    catch (error) {
    ...
    console.log(`🌐 Incoming request from IP: ${clientIp}, user data: ${JSON.stringify({
        ...
        getLocationFromIPGuide: {
            ...(await getLocationFromIPGuide(ipv4 || clientIp).catch(err => ({ error: err.message })))
        },
        server: {
            localIP: serverIP,
...
//# sourceMappingURL=get-device-info.route.js.map