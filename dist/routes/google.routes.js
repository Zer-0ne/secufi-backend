...
import { getExpiryDate, isExpired } from '../config/utils.js';
 ...
 */
googleRouter.get('/auth-url/:userId', async (req, res) => {
    try {
 ...
 * }
 */
googleRouter.post('/set-token', authenticateJWT, async (req, res) => {
    try {
 ...
 * }
 */
googleRouter.post('/get-token', authenticateJWT, async (req, res) => {
    try {
        const authHeader = req.headers.authorization;
        const token = JWTService.extractFromHeader(authHeader);
        const userId = (JWTService.decode(token)).userId;
...
googleRouter.get('/callback', async (req, res) => {
    try {
        ...
        try {
            const credentials = await googleService.exchangeCodeForTokens(codeStr, userId);
 ...
 */
googleRouter.get('/emails/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
 ...
 */
googleRouter.get('/emails/:userId/:id', async (req, res) => {
    try {
 ...
 */
googleRouter.get('/search/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
 ...
 * Clear credentials
 */
googleRouter.delete('/credentials/:userId', async (req, res) => {
    try {
        const { userId } = req.params;
        if (!userId) {
            res.status(400).json({
