...
vaultRoutes.put('/edit-asset/:assetId', authenticateJWT, async (req, res) => {
    try {
        const assetId = req.params.assetId;
        const assetData = req.body;
        const assetToUpdate = await financialDataService.getAssetById(assetId, req.user?.userId);
        ...
        for (const field of assetToUpdate?.required_fields || []) {
            isRequiredDataExist = !!(await checkValueRequired(assetData, field, req.user?.userId));
        }
        ...
        await financialDataService.updateAsset(assetId, req.user?.userId, assetData);
...
const checkValueRequired = async (assetData, requiredField, userId) => {
    const user = await prisma.user.findUnique({
        where: {
            id: userId
        }
    });
    if (assetData[requiredField] || user?.[requiredField]) {
        return true;
    }
    return false;
};
 ...
 * }
 */
vaultRoutes.get('/asset-types', authenticateJWT, async (req, res) => {
    try {
        const assetTypes = await prisma.asset.groupBy({
            by: ['type'],
            where: {
