...
export class EncryptionService {
    ...
    decrypt(encryptedData, iv) {
        try {
            ...
            const authTag = Buffer.from(encryptedData.substring(encryptedData.length - 32), // Last 32 chars
            'hex');
            // Extract actual encrypted data (everything except the auth tag)
            const encrypted = encryptedData.substring(0, encryptedData.length - 32 // Everything before the auth tag
            );
