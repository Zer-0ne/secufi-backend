...
export class GoogleService {
    ...
    userId = null;
    gmail = null;
    constructor(config, prisma, encryptionService) {
        this.prisma = prisma;
     ...
     */
    async saveCredentialsToDatabase(userId, credentials) {
        try {
            const encryptedAccessToken = this.encryptionService.encrypt(credentials.accessToken);
            const encryptedRefreshToken = this.encryptionService.encrypt(credentials.refreshToken || '');
            ...
            await this.prisma.googleCredential.create({
                data: {
                    ...
                    refresh_token_expires_at: new Date(credentials.refreshTokenExpiresAt),
     ...
     */
    async loadCredentialsFromDatabase(userId) {
        try {
            if (!userId) {
                throw new Error('User ID is required');
            }
            ...
            // Decrypt credentials
            const decryptedAccessToken = this.encryptionService.decrypt(record.encrypted_access_token, record.access_token_iv).decrypted;
            const decryptedRefreshToken = this.encryptionService.decrypt(record.encrypted_refresh_token, record.refresh_token_iv).decrypted;
            this.credentials = {
     ...
     * List all emails
     */
    async listEmails(maxResults = 10, pageToken) {
        try {
            if (!this.gmail) {
            ...
            const emailPromises = messages.map((msg) => this.getEmailById(msg.id || ''));
 ...
 */
    ...
    isTokenExpired() {
        if (!this.credentials?.accessTokenExpiresAt)
            return true;
        return (Date.now() >= this.credentials.accessTokenExpiresAt - 5 * 60 * 1000); // 5 min buffer
    }
    /**
   ...
   */
     ...
     */
    async getEmailWithAttachments(emailId, attachmentService, password) {
        try {
            if (!this.gmail) {
                throw new Error('Gmail client not initialized');
            }
            ...
            const message = response.data;
            const headers = message.payload?.headers || [];
            const getHeader = (name) => headers.find((h) => h.name === name)
                ?.value || '';
            const email = {
                id: message.id || '',
                threadId: message.threadId || '',
                ...
                subject: getHeader('Subject'),
                snippet: message.snippet || '',
                body: this.extractBodyFromPayload(message.payload) || '',
                date: getHeader('Date'),
                labels: message.labelIds || [],
                internalDate: message.internalDate || '',
            };
            // Extract attachments metadata
            const attachmentMetadata = await attachmentService.extractAttachmentMetadata(message);
            const attachments = [];
            // Download and process each attachment
            for (const attachment of attachmentMetadata) {
                try {
                    const processed = await attachmentService.downloadAndProcessAttachment(this.gmail, emailId, attachment, (this.userId || 'unknown'), password);
                    // console.log("processed data from attachments :: ", processed) // this is working fine
                    attachments.push(processed);
                    console.log(`✓ Attachment processed: ${processed.filename}`);
                }
                catch (error) {
