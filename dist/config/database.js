...
export const prisma = new PrismaClient({
    ...
    log: process.env.NODE_ENV === 'development'
        ...
        : ['error'], // Error-only logging in production
});
/**
...
export const connectDatabase = async () => {
    try {
        ...
        console.log(`   Database: ${process.env.POSTGRES_DB}`); // Database name
        console.log(`   Host: ${process.env.POSTGRES_HOST}:${process.env.POSTGRES_PORT}`); // Host and port
    }
    catch (error) {
        // Log connection failure with error details
        console.error('❌ Database connection failed:', error);
        // Re-throw error to be handled by caller (typically causes app to exit)
        throw error;
    }
};
/**
...
export const disconnectDatabase = async () => {
    try {
        ...
        // Log disconnection error (non-fatal, just for monitoring)
...
export const checkDatabaseHealth = async () => {
    try {
        // Execute a simple query to test connection (SELECT 1 is a no-op query)
        await prisma.$queryRaw `SELECT 1`;
        // Return healthy status if query succeeds
        return {
            status: 'healthy', // Database is functioning normally
            connected: true // Connection is active
        };
    }
    catch (error) {
        // Return unhealthy status if query fails
        return {
            status: 'unhealthy', // Database has issues
            connected: false, // Connection is not active
