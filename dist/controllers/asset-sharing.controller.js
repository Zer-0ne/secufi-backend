...
export class AssetSharingController {
     ...
     */
    static async shareAsset(req, res) {
        try {
            const userId = req.user?.userId;
            const { assetId, 
            // familyId: requestedFamilyId,
            sharedWithUserId, canView = true, canEdit = false, canDelete = false, expiresAt, notes, } = req.body;
     ...
     * @throws {500} For internal server errors during database operations
     */
    static async getSharedAssets(req, res) {
        try {
            const userId = req.user?.userId;
            ...
            const assetsWithPermissions = await Promise.all(sharedAssets.map(async (shared) => {
                const userMembership = memberships.find((m) => m.family_id === shared.family_id);
                if (!userMembership)
                    return null;
     ...
     * @throws {500} For internal server errors during database operations
     */
    static async getMySharedAssets(req, res) {
        try {
            const userId = req.user?.userId;
            const sharedAssets = await prisma.sharedAsset.findMany({
                where: {
     ...
     * @throws {500} For internal server errors during database operations
     */
    static async revokeAssetSharing(req, res) {
        try {
            const userId = req.user?.userId;
            ...
            if (sharedAsset.shared_by_user_id !== userId &&
...
//# sourceMappingURL=asset-sharing.controller.js.map